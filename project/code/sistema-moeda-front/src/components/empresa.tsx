...
function Empresa(id: Number) {
  ...
  const getEmpresa = async () => {
    const res = await axios.get('http://localhost:8080/api/company/' + location.state.id)
    setEmpresa(res.data)
  }

  const getVantagens = async () => {
    const res = await axios.get('http://localhost:8080/api/advantage')
    setVantagens(res.data)
  }

  const createVantagem = async () => {
    newVantagem.companyId = location.state.id
  ...
  return (
    <>
     <h1>Hello Empresa {empresa.name} </h1>
